...
////////
  ...
  $html = file_get_contents('http://www.reuters.com/news/archive/worldNews?date=today');
...
$content=array('texts'=>array());
  ...
  foreach ($links as $link){
    if($link->getAttribute('class')=='story-title') array_push($content['texts'],$link->nodeValue);
      $i++;
      if($i==10) break;
  }
  $options = array(
  ...
  foreach ($links as $index=>$link){
    ...
    if($pos>=$neg) $kind='<img src="images/plus.png" style="width:50px;height:50px;">';
      ...
      if($link->getAttribute('class')=='story-title') echo ( '<h6 class="latest_news">'.'
